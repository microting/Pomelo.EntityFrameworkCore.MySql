...
""");
        ...
        public async Task IsRing(bool async)
        {
            ...
            AssertSql(
                AppConfig.ServerVersion.Supports.SpatialFunctionAdditions
                    ? """
SELECT `l`.`Id`, CASE
    WHEN `l`.`LineString` IS NULL THEN NULL
    ELSE ST_IsRing(`l`.`LineString`)
END AS `IsRing`
FROM `LineStringEntity` AS `l`
"""
                    : """
