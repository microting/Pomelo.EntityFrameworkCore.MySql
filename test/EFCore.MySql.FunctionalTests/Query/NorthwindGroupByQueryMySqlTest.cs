...
""");
        ...
        //
        // On MySQL the base assertion fails outright with a result mismatch (for example the group for customer
        // LACOR yields a different order), because the query selects a whole entity from a group without an
        // aggregate and MySQL does not guarantee which row of the group is returned. On MariaDB the base assertion
        // passes, so the test still provides coverage there.
        //
        // The SQL is deliberately not asserted: this override only ever executes on MariaDB, whereas every other
        // baseline in this class is captured against MySQL, so a baseline here would be inconsistent with the rest
        // of the file and could not be regenerated by the MySQL-based rewrite workflow.
